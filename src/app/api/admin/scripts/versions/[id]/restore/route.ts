...
export async function POST(
    ...
    try {
        ...
        // Cascading deletes on call_nodes should handle children, but let's be safe/explicit if needed.
        // Assuming ON DELETE CASCADE is set up in schema. If not, we'd need to delete children first.
        // In Phase 1 migration plan, it wasn't explicitly stated but standard practice.
        // I'll assume Cascade for now to keep it atomic-ish, or delete manually.
        // Let's try deleting nodes first.
        ...
        if (nodes && nodes.length > 0) {
            // Strip out any potentially problematic fields if necessary, but raw insert should work if schema matches
